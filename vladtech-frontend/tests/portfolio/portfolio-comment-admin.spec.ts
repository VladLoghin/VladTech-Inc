...
test.describe('Portfolio Comment - Admin - Desktop', () => {
  test.beforeEach(async ({ page }) => {
    await page.setViewportSize({ width: 1920, height: 1080 });
  });

  test('admin can add comment on portfolio item', async ({ page, loginAs }) => {
    // Login as admin
    ...
    const commentText = `Admin test comment ${Date.now()}`;
    ...
    // Verify the comment appears with the admin's name
    ...
    await expect(lastComment.locator('text=30boombap30')).toBeVisible();
  });
});

test.describe('Portfolio Comment - Admin - Mobile', () => {
  test.beforeEach(async ({ page }) => {
    await page.setViewportSize({ width: 390, height: 844 });
  });

  test('admin can add comment on portfolio item - Mobile', async ({ page, loginAs }) => {
    // Login as admin
    await loginAs('realAdmin');
    ...
    // Generate unique comment text
    const commentText = `Admin mobile test ${Date.now()}`;
