    ...
    try {
      ...
      if (mode === 'remove') {
        ...
        if (query.trim()) {
          ...
          const roleUserIds = new Set((roleUsersResponse.data.users || []).map((u) => u.user_id));
  ...
  const handleRoleAction = async (userId, userName) => {
    ...
    try {
      ...
      if (mode === 'assign') {
        ...
        setSuccess(
          t('roleManager.roleAssignedTo', { role: config.label, name: userName || userId })
        );
      } else {
        ...
        setSuccess(
          t('roleManager.roleRemovedFrom', { role: config.label, name: userName || userId })
        );
      }
      ...
      <div className="bg-white rounded-xl shadow-2xl max-w-5xl w-full max-h-[90vh] flex flex-col">
            ...
            <p className="text-sm text-black/60 mt-1">
              {mode === 'history'
                ...
                : mode === 'assign'
                  ...
                  : t('roleManager.usersWithRole', {
                      count: totalUsers,
                      role: roleConfig[selectedRole].label,
                    })}
            </p>
          </div>
          ...
          <div>
            ...
            <div className="flex border-2 border-black rounded-lg overflow-hidden w-fit">
              <button
                ...
                className={`px-6 py-2 font-semibold transition-all flex items-center gap-2 ${
                  mode === 'assign'
                    ? 'bg-black text-white'
                    ...
                    <div className="flex items-center gap-4">
                      <div
                        className={`w-10 h-10 rounded-full flex items-center justify-center ${
                          log.action === 'ASSIGNED' ? 'bg-green-400' : 'bg-red-400'
                        }`}
                      >
                        {log.action === 'ASSIGNED' ? (
                      ...
                      <div>
                        <p className="font-semibold">
                          ...
                          >
                            {log.action === 'ASSIGNED'
                              ? t('roleManager.assigned')
                              : t('roleManager.removed')}
                          </span>{' '}
                          {log.roleName} {t('roleManager.role')}
                        </p>
                        <p className="text-sm text-black/80">{log.userName || log.userId}</p>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <p className="font-semibold text-lg">
                        {user.name || t('roleManager.noName')}
                      </p>
        ...
        <div className="border-t border-black/10 p-6 flex items-center justify-between">
          ...
          <div className="text-sm text-black/60">
            {mode === 'history'
              ? t('roleManager.pageOfTotal', {
                  current: historyPage + 1,
                  total: totalHistoryPages || 1,
